...
/* ============================================================
   AJAX: FETCH PHARMACY CONSUMABLES BASED ON WORKING LOCATION
=============================================================== */
...
/* ============================================================
   AJAX: PICK PHARMACY CONSUMABLE — DEDUCT FROM pharmacy_consumable_stock
=============================================================== */
if (isset($_POST['pick']) && isset($_POST['id']) && isset($_POST['qty'])) {
    ...
    try {
        $stmtCheck = $mysqli->prepare("SELECT quantity FROM pharmacy_consumable_stock WHERE id = ? AND campus_id = ? LIMIT 1");
        ...
        $newQty = $stockQty - $qty;
        $stmtUpd = $mysqli->prepare("UPDATE pharmacy_consumable_stock SET quantity = ? WHERE id = ? AND campus_id = ?");
...
<html lang="en">
    ...
    <body>
        ...
        <div id="wrapper">
            ...
            <!-- Left Sidebar: Pharmacy Navigation -->
            ...
            <div class="content-page">
                <div class="content">
                    <div class="container-fluid">
                        ...
                        <h3 class="mb-4">Pharmacy Consumables (Pick Items)</h3>
    ...
    <?php else: ?>
        ...
        <div class="table-responsive">
            <table class="table table-bordered" id="pharmacy_consumable_table">
                ...
                <tbody id="pharmacy_consumable_results">
        ...
        <script>
        async function fetchPharmacyConsumables() {
            const res = await fetch('pharmacy_consumables_pick.php?ajax=1');
            const data = await res.json();
            const tbody = document.getElementById('pharmacy_consumable_results');
            if (!tbody) return;
            tbody.innerHTML = '';
                        ...
                        <tr>
                            ...
                            <td>
                                <input type="number" min="1" max="${row.quantity}" value="1"
                                       ...
                                       style="width:80px" id="pharmacy_pick_qty_${row.id}">
                                <button type="button" class="btn btn-sm btn-success ms-1" onclick="pickPharmacyConsumable(${row.id}, ${row.quantity})">Pick</button>
                            </td>
                        </tr>
                    `;
                });
        ...
        async function pickPharmacyConsumable(id, maxQty) {
            const input = document.getElementById('pharmacy_pick_qty_' + id);
            ...
            const res = await fetch('pharmacy_consumables_pick.php', {
                method: 'POST',
                body: formData
            });
            const data = await res.json();
            alert(data.message || (data.status === 'success' ? 'Operation completed.' : 'Error'));
            fetchPharmacyConsumables();
        }

        document.addEventListener('DOMContentLoaded', fetchPharmacyConsumables);
        </script>
    <?php endif; ?>
